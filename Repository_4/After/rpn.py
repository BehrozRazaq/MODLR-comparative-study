...
class RPNplus(tf.keras.Model):
    ...
    def __init__(self):
        ...
        self.region_proposal_conv1 = tf.keras.layers.Conv2D(
            256,
            kernel_size=[5, 2],
        ...
        self.region_proposal_conv2 = tf.keras.layers.Conv2D(
            512,
            kernel_size=[5, 2],
            activation=tf.nn.relu,
            padding="same",
        ...
        self.region_proposal_conv3 = tf.keras.layers.Conv2D(
            512,
            kernel_size=[5, 2],
            activation=tf.nn.relu,
            padding="same",
        ...
        self.bboxes_conv = tf.keras.layers.Conv2D(
            36, kernel_size=[1, 1], padding="same", use_bias=False
        )
        ## Output Scores layer
        self.scores_conv = tf.keras.layers.Conv2D(
            18, kernel_size=[1, 1], padding="same", use_bias=False
        )
    ...
    def call(self, x, training=False):
        ...
        region_proposal = tf.concat(
            [pool3_p, pool4_p, pool5_p], axis=-1
        )  # [1, 45, 60, 1280]
