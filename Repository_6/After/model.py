...
from utils import convert_idx_to_token_tensor  # type: ignore
...
class Transformer:
    ...
    def __init__(self, hp, token2idx, idx2token):
        self.hp = hp
        self.token2idx, self.idx2token = token2idx, idx2token
    ...
    def decode(self, ys, memory, src_masks, training=True):
        ...
        with tf.variable_scope("decoder", reuse=tf.AUTO_REUSE):
            ...
            dec = tf.nn.embedding_lookup(
                self.embeddings, decoder_inputs
            )  # (N, T2, d_model)
    ...
    def eval(self, xs, ys):
        ...
        decoder_inputs = (
            tf.ones((tf.shape(xs[0])[0], 1), tf.int32) * self.token2idx["<s>"]
        )
